...
export function SchemaImportSidebar({
  onSchemaImport,
}: SchemaImportSidebarProps) {
  const [schemaInput, setSchemaInput] = useState("");
  const [aiSchemaInput, setAiSchemaInput] = useState("");
  const [isAiSchemaLoading, setIsAiSchemaLoading] = useState(false);
  const [isParsing, setIsParsing] = useState(false);
  const [parseError, setParseError] = useState<string | null>(null);
  const [parsedPreview, setParsedPreview] = useState<FormField[] | null>(null);
  const [isAISchemaEnabled, setisAISchemaEnabled] = useState(false);
  ...
  const handleParse = async () => {
    ...
    try {
      ...
      if (fields.length === 0) {
        setParseError(
          "No fields found in schema. Make sure it's a valid z.object() schema."
        );
        return;
      }
  ...
  const handleAiSchemaGeneration = useCallback(async () => {
    try {
      setIsAiSchemaLoading(true);
      if (aiSchemaInput.trim().length === 0)
        throw Error("Please enter a prompt");

      const data = await aiService.generateSchema(aiSchemaInput);
      if (!data) throw Error("No schema returned");
  
      const fields = parseZodSchemaString(data);
      if (fields.length === 0)
        throw Error("Schema generated but no fields could be parsed");
  
      onSchemaImport(fields);
      toast.success(`Generated and imported ${fields.length} fields!`);
  
      setAiSchemaInput("");
      setisAISchemaEnabled(false);
    } catch (error) {
      toast.error(
        error instanceof Error ? error.message : "Failed to generate schema"
      );
    } finally {
      setIsAiSchemaLoading(false);
    }
  }, [aiSchemaInput, onSchemaImport]);
    ...
    <div className="w-80 border-l bg-muted/30 flex flex-col h-full">
      ...
      <div className="p-4 border-b bg-background">
        <div className="flex items-center gap-2 mb-1">
          ...
          <h3 className="font-semibold text-sm">Import/Create Schema</h3>
        </div>
        <p className="text-xs text-muted-foreground">
          Paste your Zod schema to auto-generate form fields or Create the
          schema using Ai
        </p>
      </div>
      ...
      {isAISchemaEnabled ? (
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          <Textarea
            onChange={(e) => setAiSchemaInput(e.target.value)}
            className="font-mono text-xs min-h-[200px] resize-none"
            placeholder="please enter some details, like what type of schema you wanted to create e.g: Create an Schema for Signup Form"
          />
          <Button
            className="w-full"
            variant="default"
            onClick={handleAiSchemaGeneration}
            disabled={isAiSchemaLoading}
          >
            {isAiSchemaLoading && <Spinner data-icon="inline-start" />}
            Create Schema
          </Button>
          <Separator />
          <div className="w-full">
            <Button
              className="w-full"
              onClick={() => setisAISchemaEnabled(false)}
              variant="secondary"
              disabled={isAiSchemaLoading}
            >
  ...
  phone: z.string(),
          ...
          {/* {!parsedPreview && (
            <>
              ...
              <Button
                className="w-full"
                onClick={() => setisAISchemaEnabled(true)}
              >
      ...
      <div className="p-4 border-t bg-muted/50">
        <p className="text-[10px] text-muted-foreground">
          <strong>Tips:</strong> Field names auto-detect UI types (email → email
          input, phone → phone input, etc.)
        </p>
      </div>
    </div>
  );
